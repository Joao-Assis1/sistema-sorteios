            ...
            >
              <div class="px-6 pb-6">
                ...
                <button
                  ...
                  class="w-full mt-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition"
                >
                  <svg
                    v-if="loadingExport"
                    class="animate-spin h-5 w-5"
...
// Exportar participantes em CSV (com autenticação)
const exportParticipantsCSV = async () => {
  loadingExport.value = true;
  try {
    const response = await api.get("/admin/export-participants", {
      responseType: "blob",
    });
    ...
    Swal.fire({
      icon: "error",
      title: "Erro",
      text: "Não foi possível exportar os participantes.",
